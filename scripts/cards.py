...
These are files in your own repo, so they render as long as GitHub renders.
    ...
    python scripts/cards.py --user emmi-lili --out assets
...
LANG_COLOR = {
    ...
    "Blade": "#f7523f", "Twig": "#c1d026", "Dockerfile": "#384d54",
}


def br(value: int) -> str:
    """Formata milhar no padrao brasileiro: 1204 -> 1.204."""
...
def render_stats(user, stats, theme):
    ...
    out = [
        ...
        f'fill="{c["muted"]}">em números</text>',
...
def main(argv=None):
    ...
    tiles = [("Estrelas", br(stars)),
             ("Repositórios", br(user["public_repos"])),
             ("Seguidores", br(user["followers"]))]

    contrib = fetch_contributions(args.user, token)
    if contrib:
        total, current, longest = contrib
        tiles += [("Contribuições (1 ano)", br(total)),
                  ("Sequência atual", br(current)),
                  ("Maior sequência", br(longest))]
    else:
